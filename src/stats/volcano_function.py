...
def Make_Volcano(volc_data, list_samp_1, list_samp_2):
    volc_data["Mean_int_1"] = volc_data[list_samp_1].mean(
        axis=1
    )  # Mean intensities in sample 1
    volc_data["Mean_int_2"] = volc_data[list_samp_2].mean(
        axis=1
    )  # Mean intensities in sample 2
    volc_data["fc"] = pd.DataFrame(
        np.log2(volc_data["Mean_int_2"] / volc_data["Mean_int_1"])
    )  # Calculates log2(FC) values
    volc_data["p"] = stats.ttest_ind(
        volc_data[list_samp_1], volc_data[list_samp_2], axis=1
    )[
        1
    ].tolist()  # Calculates p-values
    ####
