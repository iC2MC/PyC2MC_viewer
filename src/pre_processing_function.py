...
def pre_processing(self):
    ...
    if self.df_type == "Attributed":
        classes, classes_concat, df, heteroatoms = split_classes(
            self.df, self.heteroatoms
        )
    ...
    if self.df_type == "PyC2MC_merged":
        classes, classes_concat, df, heteroatoms, volc_data = split_classes_merged(
            self.df, self.heteroatoms, self.pca_data
        )
        df, dbe_both, C_both, N_both, O_both, S_both, dbe_odd, dbe_even = (
            formating_merged(df)
        )
        self.df = df
        self.classes = classes
    ...
    if self.df_type == "PyC2MC_fusion":
        classes, classes_concat, df, heteroatoms = split_classes_fusionned(
            self.df, self.heteroatoms
        )
        df, dbe_both, C_both, N_both, O_both, S_both, dbe_odd, dbe_even = formating(df)
        self.df = df
...
def split_classes(df, heteroatoms):
    ...
    for i in range(1, len(heteroatoms) + 1):
        for het_selected in itertools.combinations(heteroatoms, i):
            ...
            percent_value = (
                intensity_family / total_intensity * 100
            )  # % intensité de la famille
            if percent_value > 0:
                value = [percent_value, nb_peaks]
    ...
    classes_concat["classes"] = (
        classes["variable"]
        + " "
        + classes["number"].astype(int).astype(str)
        + " / "
        + classes["value"].round(2).astype(str)
        + "%"
    )
...
def split_classes_merged(df, heteroatoms, pca_data):
    ...
    df["Normalized_intensity"] = (
        df["summed_intensity"] / max(df["summed_intensity"]) * 100
    )
    ...
    while n < len(heteroatoms):
        ...
        while p < n_sample:
            temp_moyenne.append(
                sum(rel_intens.iloc[:, p].astype(float))
                * 100
                / (total_intensity / n_sample)
            )
    ...
    if "PC1" in df.columns:
        classe_index = df.iloc[
            :,
            df.columns.get_loc("Normalized_intensity")
            + 1 : df.columns.get_loc("H/C")
            - 1,
        ].copy()
    else:
        classe_index = df.iloc[
            :, df.columns.get_loc("Normalized_intensity") + 1 :
        ].copy()
    ...
    classes_concat["classes"] = (
        ...
        + classes["number"].astype(int).astype(str)
        + " / "
        + classes["value"].astype(float).round(2).astype(str)
        + "%"
        + " / "
        + classes["std_dev_rel"].astype(float).round(1).astype(str)
        + "%"
    )
...
def split_classes_merged_unattributed(df, pca_data):
    rel_columns = [col for col in df.columns if col.startswith("rel_")]
    df["summed_intensity"] = df[rel_columns].sum(axis=1)
    df["Normalized_intensity"] = (
        df["summed_intensity"] / max(df["summed_intensity"]) * 100
    )
...
def split_classes_fusionned(df, heteroatoms):
    ...
    while n < len(heteroatoms):
        ...
        while p < n_sample:
            temp_moyenne.append(
                sum(rel_intens.iloc[:, p].astype(float)) * 100 / total_intensity
            )
            temp_nb.append(sum(rel_intens.iloc[:, p].astype(float) != 0))
            p = p + 1
        percent_value = Average(temp_moyenne)
...
def formating_merged(df):
    ...
    for i in range(len(all_DBE)):
        intensity_DBE = (
            100 * sum(df["summed_intensity"].loc[df.DBE == all_DBE.values[i]]) / tot_int
        )
        if intensity_DBE:
    ...
    if "C" in df:
        ...
        for i in range(len(all_C)):
            intensity_C = (
                100 * sum(df["summed_intensity"].loc[df.C == all_C.values[i]]) / tot_int
            )
            if intensity_C:
    ...
    if "N" in df:
        ...
        for i in range(len(all_N)):
            intensity_N = (
                100 * sum(df["summed_intensity"].loc[df.N == all_N.values[i]]) / tot_int
            )
            if intensity_N:
    ...
    if "O" in df:
        ...
        for i in range(len(all_O)):
            intensity_O = (
                100 * sum(df["summed_intensity"].loc[df.O == all_O.values[i]]) / tot_int
            )
            if intensity_O:
    ...
    if "S" in df:
        ...
        for i in range(len(all_S)):
            intensity_S = (
                100 * sum(df["summed_intensity"].loc[df.S == all_S.values[i]]) / tot_int
            )
            if intensity_S:
