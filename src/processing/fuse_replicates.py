...
def fuse_replicates(names, replicates, isotopes_dict):
    ...
    if len(names[0]) == 1:
        ...
        while n < len(names[0]):
            ...
            try:
                temp_data["%s" % n] = load_MS_file(names[0][n], isotopes_dict).df
            except:
                temp_data["%s" % n] = load_MS_file(names[0][n], isotopes_dict).df

            temp_data["%s" % n] = temp_data["%s" % n].sort_values(
                by=["normalized_intensity"], ascending=True
            )
            temp_data["%s" % n]["relative_intens"] = (
                temp_data["%s" % n]["normalized_intensity"]
                / sum(temp_data["%s" % n]["normalized_intensity"])
            ) * 100
        ...
        intens_abs_merged = reduce(
            lambda left, right: pandas.merge(left, right, on=["m/z"], how="outer"),
            data_frames_abs_intens,
        ).fillna("0")
