...
def merge_non_attributed(
    names: str, tol: float, min_rel_intens: float = None, callback: callable = None
):
    """
    ...
    for i in names[0]:
        ...
        temp_data: pandas.DataFrame = load_MS_file(filename).df[
            ["m/z", "absolute_intensity"]
        ]
        temp_data.rename(columns={"m/z": "mz", "absolute_intensity": "I"}, inplace=True)
        temp_data["Rel_intens"] = (temp_data["I"] / max(temp_data["I"])) * 100
    ...
    for i in range(len(names[0]) - 1):
        ...
        for n, row in left.iterrows():
            ...
            if callback != None:
                progress = 100 * (
                    (i) / (len(names[0]) - 1) + n / (len(left) * (len(names[0]) - 1))
                )
                callback(progress)
        ...
        temp_merged = temp_merged.merge(
            right, on="idx", how="outer", suffixes=("_l", "_r")
        )
        ...
        if i < 1:
            ...
            temp_merged = temp_merged.assign(
                Rel_intens_y=lambda x: x["Rel_intens_y"].fillna(x["Rel_intens"])
            )
        else:
            temp_merged = temp_merged.assign(I_y=lambda x: x["I_l"].fillna(x["I_r"]))
            temp_merged = temp_merged.assign(
                Rel_intens_y=lambda x: x["Rel_intens_l"].fillna(x["Rel_intens_r"])
            )
        temp_merged = temp_merged.rename(
            columns={"mz": "mz", "I_y": f"I_{i}", "Rel_intens_y": f"Rel_intens_{i}"}
        )
        if i < 1:
            temp_merged = temp_merged.drop(
                ["mz_l", "mz_r", "idx", "I", "Rel_intens"], axis=1
            )
        else:
            temp_merged = temp_merged.drop(
                ["mz_l", "mz_r", "idx", "I_l", "Rel_intens_l", "I_r", "Rel_intens_r"],
                axis=1,
            )
