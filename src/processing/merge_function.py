...
def merge_file(names, isotopes_dict):
    ...
    if len(names) == 1:
        ...
        sumformula_splitted = [
            chemparse.parse_formula(formula) for formula in sumformula
        ]
        ...
        while n < len(names):
            data["%s" % n]["molecular_formula"] = data["%s" % n][
                "molecular_formula"
            ].replace(" ", "", regex=True)
            data_frames_abs_intens.append(
                data["%s" % n][["molecular_formula", "absolute_intensity"]]
            )
            data_frames_rel_intens.append(
                data["%s" % n][["molecular_formula", "normalized_intensity"]]
            )
            n = n + 1
        ...
        intens_abs_merged = reduce(
            lambda left, right: pandas.merge(
                left, right, on=["molecular_formula"], how="outer"
            ),
            data_frames_abs_intens,
        ).fillna(0)
        intens_rel_merged = reduce(
            lambda left, right: pandas.merge(
                left, right, on=["molecular_formula"], how="outer"
            ),
            data_frames_rel_intens,
        ).fillna(0)
        intens_abs_merged = intens_abs_merged.drop_duplicates(
            subset="molecular_formula"
        )
        intens_rel_merged = intens_rel_merged.drop_duplicates(
            subset="molecular_formula"
        )
...
def merge_for_compare(data, name, isotopes_dict):
    ...
    data_frames_std_rel = []
    while n < len(data):
        data[n]["molecular_formula"] = data[n]["molecular_formula"].replace(
            " ", "", regex=True
        )
        if "std_dev_rel" in data[n]:
            data_frames_abs_intens.append(
                data[n][["molecular_formula", "absolute_intensity"]]
            )
            data[n]["relative_intens"] = (
                data[n]["absolute_intensity"] / sum(data[n]["absolute_intensity"])
            ) * 100
            data_frames_rel_intens.append(
                data[n][["molecular_formula", "relative_intens"]]
            )
    ...
    intens_rel_merged.reset_index(inplace=True, drop=True)
    intens_rel_merged = pandas.DataFrame(intens_rel_merged).astype("float")
    merged["summed_intensity"] = intens_rel_merged.iloc[
        :, intens_rel_merged.columns != "molecular_formula"
    ].sum(axis=1)
